...
def resolve_path(path):
    """
    Resolve a user-given path so files OUTSIDE the toolkit's own folder work
    correctly, regardless of where the script itself lives:
      - expands ~ (home directory)
      - expands environment variables like %USERPROFILE% / $HOME
      - converts to an absolute path based on the current working directory
        (i.e. the directory the user ran the command from - not the script's
        directory), so both relative paths ("../samples/chall.png") and full
        absolute paths ("C:\\Users\\gus\\Downloads\\chall.png" or
        "/home/gus/Downloads/chall.png") work.
    """
    if not path:
        return path
...
def require_file(path):
    """Resolve + validate a path. Prints a clear error (with the full
    resolved path) if the file doesn't exist, so it's obvious what was
    actually looked up when analyzing files outside the tool's own folder."""
    resolved = resolve_path(path)
    if not os.path.isfile(resolved):
        cprint(f"[!] File not found: {resolved}", C.RED)
        if resolved != path:
            cprint(f"    (resolved from input: {path})", C.DIM)
        return None
    return resolved
...
class Reverse:
    ...
    def disasm(hexbytes_or_file, arch="x86_64", offset=0):
        ...
        resolved = resolve_path(hexbytes_or_file)
...
class CTFShell(cmd.Cmd):
    ...
    def preloop(self):
        ...
        print_modules_table()
        cprint("  Type e.g. 'crypto caesar synt{uryyb}' | 'help' for all commands | 'exit' to quit.", C.DIM)
        print()
